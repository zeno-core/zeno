...
pub fn scan_prefix_materialized_profiled(
    ...
    return metrics.call_with_latency(&db.state, scan_ops.scan_prefix_materialized_profiled, .{ &db.state, allocator, prefix, shard_chunk_size });
}
...
///
/// Time Complexity: O(s + r * (k + s + v)), where `s` is shard count, `r` is emitted result size, `k` is ART seek work for one chunk refill, and `v` is total cloned value size.
///
/// Allocator: Allocates owned entry keys and values plus result storage and bounded per-shard chunk scratch through `allocator`.
///
/// Ownership: Returns a caller-owned `ScanResult` plus profiling counters by value. The caller must later call `deinit` on the returned `ScanResult`.
...
pub fn scan_prefix_materialized_winner_selected_profiled(
    ...
    return metrics.call_with_latency(&db.state, scan_ops.scan_prefix_materialized_winner_selected_profiled, .{ &db.state, allocator, prefix, shard_chunk_size });
}
...
pub fn scan_prefix_materialized_fixed_heap_profiled(
    ...
    return metrics.call_with_latency(&db.state, scan_ops.scan_prefix_materialized_fixed_heap_profiled, .{ &db.state, allocator, prefix, shard_chunk_size });
}
...
pub fn scan_prefix_materialized_bulk_collect_profiled(
    ...
    return metrics.call_with_latency(&db.state, scan_ops.scan_prefix_materialized_bulk_collect_profiled, .{ &db.state, allocator, prefix });
}
...
test "scan_prefix_materialized_from_in_view_profiled matches full scan ordering" {
    const testing = std.testing;
...
test "scan_prefix_materialized_profiled matches full scan ordering" {
    const testing = std.testing;
    ...
    var profiled = try scan_prefix_materialized_profiled(db, testing.allocator, "alpha", 2);
    defer profiled.result.deinit();
...
test "scan_prefix_materialized_winner_selected_profiled matches full scan ordering" {
    const testing = std.testing;
    ...
    var profiled = try scan_prefix_materialized_winner_selected_profiled(db, testing.allocator, "alpha", 2);
    defer profiled.result.deinit();
...
test "scan_prefix_materialized_fixed_heap_profiled matches full scan ordering" {
    const testing = std.testing;
    ...
    var profiled = try scan_prefix_materialized_fixed_heap_profiled(db, testing.allocator, "alpha", 2);
    defer profiled.result.deinit();
...
test "scan_prefix_materialized_bulk_collect_profiled matches full scan ordering" {
    const testing = std.testing;
    ...
    var profiled = try scan_prefix_materialized_bulk_collect_profiled(db, testing.allocator, "alpha");
    defer profiled.result.deinit();
...
test "scan_prefix_materialized_bulk_visit_profiled matches full scan ordering" {
    const testing = std.testing;
