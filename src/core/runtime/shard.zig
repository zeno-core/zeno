...
//! Allocator: Uses the engine base allocator for owned key bytes and nested value storage.
...
pub const Shard = struct {
    ...
    ///
    /// Time Complexity: O(n + b), where `n` is retained key count and `b` is total bytes owned by nested values.
    ///
    /// Allocator: Does not allocate; frees owned keys and nested values through `base_allocator`.
    ///
